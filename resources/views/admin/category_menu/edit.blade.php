        ...
        <div class="col-md-12">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb pl-0">
              ...
              <li class="breadcrumb-item"><a href="#">{{('menu.Category Menu')}}</a></li>
              <li class="breadcrumb-item active" aria-current="page">{{__('menu.Edit Category Menu')}}</li>
            </ol>
          </nav>
        </div>
        ...
        <div class="col-xl-12 col-md-12">
          <div class="ms-panel ms-panel-fh">
            <div class="ms-panel-header">
              <h6>{{__('menu.Edit Category Menu')}}</h6>
            </div>
              ...
              <form id="submitForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" class="form-control" name="category_name_ar" id="category_name_ar" value="{{$category->category_name_ar}}" placeholder="Category_Name(Arabic)">
                        <div class="invalid-feedback" style="display:contents;" id="category_name_ar_error"></div>
                    </div>

                    <div class="col-md-12 mb-3">
                        <label>{{__('menu.Category Name(English)')}}</label>
                        <input type="text" class="form-control" name="category_name_en" id="category_name_en" value="{{$category->category_name_en}}" placeholder="Category_Name(English)">
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <select name="branch_id[]" id="branch_id" class="branch_id" multiple onchange="console.log(Array.from(this.selectedOptions).map(x=>x.value??x.text))" >
                            @foreach($branches as $branch)
                                <option @foreach($branches_categories as $cat) @if($branch->id == $cat->branch_id) selected @endif  @endforeach value="{{ $branch->id }}">{{$branch->branch_name_ar}}</option>
                            @endforeach
                        </select>
...
<script type="application/javascript">
  $(document).ready(function(){
    ...
    $('#submitForm').submit(function(e){
        ...
        $.ajax({
            url:"{{ url('admin/category/menu/update/' . $category->id) }}",
            type:"POST",
            data:formData,
            processData: false,
            contentType: false,
            success:function(data){
                Swal.fire({
                    title: 'لقد تم تعديل الصنف بنجاح !',
